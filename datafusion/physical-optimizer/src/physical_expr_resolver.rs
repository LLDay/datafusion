...
//! This file implements the `PhysicalExprResolver` physical optimization rule.
//! The rule ensures that the physical plan is prepared for placeholder resolution by wrapping it
//! in a [`TransformPlanExec`] with a [`ResolvePlaceholdersRule`] if the plan contains any unresolved placeholders.
//! The actual resolution happens during execution.
        ...
        match self.phase {
            ...
            PhysicalExprResolverPhase::Post => {
                    ...
                    plan.as_any().downcast_ref::<TransformPlanExec>()
                {
                    ...
                    let transformer = TransformPlanExec::new(
                        plan,
