...
use datafusion_physical_plan::plan_transformer::{TransformPlanExec, TransformationRule};
...
impl TransformationRule for ResetAllRule {
    fn name(&self) -> &str {
        "ResetAllRule"
    }
...
impl TransformationRule for ResetByNameRule {
    fn name(&self) -> &str {
        "ResetByNameRule"
    }
...
fn benchmark_with_planned_rewriter(
    group: &mut BenchmarkGroup<'_, WallTime>,
    batch_label: &str,
    plan: &Arc<dyn ExecutionPlan>,
    rules: &[Box<dyn TransformationRule>],
    ...
    group.bench_function(
        ...
        |b| {
            b.iter_batched(
                ...
                |(plan, rules)| {
                    let transformer = TransformPlanExec::new(plan, rules).unwrap();
    ...
    group.bench_function(
        ...
        |b| {
            ...
            let transformer = Arc::new(TransformPlanExec::new(plan, rules).unwrap());
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    for depth in depths {
        ...
        for count in rules_count {
            ...
            benchmark_with_planned_rewriter(
                &mut group,
                "reset_all",
                &plan,
                &reset_all_rules,
