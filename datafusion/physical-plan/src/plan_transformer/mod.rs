...
//! [`TreeNodeRewriter`]. This approach is useful when the decision to transform an
//! [`ExecutionPlan`] is complex or depends on the overall tree structure discovered during the
//! first pass.
//!
...
/// traversal. In the first pass, the [`TransformationRule::matches`] method is used to identify
/// nodes that require transformation. In the second pass, the [`TransformationRule::rewrite`]
/// method is used to apply the transformation.
pub trait TransformationRule: Send + Sync + Debug {
    ...
    /// Clones this rule.
    fn clone_box(&self) -> Box<dyn TransformationRule>;
...
impl TransformationPlanner {
    /// Create a new planner with the given rules.
    fn new(rules: Vec<Box<dyn TransformationRule>>) -> Self {
        Self {
            cursor: 0,
            rules,
            plans: Vec::new(),
...
impl ExecutionPlan for TransformPlanExec {
    ...
    fn with_new_children(
        ...
        Ok(Arc::new(TransformPlanExec::new(
...
mod tests {
    ...
    impl TransformationRule for MockRule {
        fn name(&self) -> &str {
            &self.name
        }
    ...
    fn test_has_rule() -> Result<()> {
        ...
        let exec = TransformPlanExec::new(input, vec![resolve_rule])?;
    ...
    fn test_add_rules_merge() -> Result<()> {
        ...
        let exec = TransformPlanExec::new(Arc::clone(&input), vec![rule_a.clone()])?;
        ...
        let exec = TransformPlanExec::new(
            Arc::clone(&input),
            vec![
                rule_a.clone(),
                rule_b.clone(),
        ...
        let exec = TransformPlanExec::new(Arc::clone(&input), vec![rule_a])?
