...
use crate::{ExecutionPlan, plan_transformer::TransformationRule};

/// A transformation rule that replaces [`PlaceholderExpr`] with actual values.
///
/// This rule is applied to the physical plan when actual parameter values are provided
/// in the [`TaskContext`]. It traverses the plan and replaces any placeholders found
/// in physical expressions with their corresponding literal values.
...
impl TransformationRule for ResolvePlaceholdersRule {
    fn name(&self) -> &str {
        "ResolvePlaceholders"
    }
